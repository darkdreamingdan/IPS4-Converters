...
class _Mybb extends \IPS\convert\Software
{
	...
	public static function canConvert()
	{
		return array(
			...
			'convert_warn_actions'			=> array(
				'table'							=> 'warninglevels',
				'where'							=> NULL
			),
			'convert_warn_reasons'			=> array(
				'table'							=> 'warningtypes',
				'where'							=> NULL
			),
			...
			'convert_question_and_answers'	=> array(
				'table'							=> 'questions',
				'where'							=> NULL
			)
		);
		...
		switch( $method )
		{
			...
			case 'convert_profile_fields':
				...
				foreach( $this->db->select( '*', 'profilefields' ) AS $field )
				{
					...
					$return['convert_profile_fields']["map_pfield_{$field['fid']}"] = array(
			...
			case 'convert_groups':
				...
				foreach( $this->db->select( '*', 'usergroups' ) AS $group )
				{
					...
					$return['convert_groups']["map_group_{$group['gid']}"] = array(
						'field_class'		=> 'IPS\\Helpers\\Form\\Select',
						'field_default'		=> NULL,
						'field_required'	=> FALSE,
						'field_extra'		=> array( 'options' => $options ),
				...
				$return['convert_groups']['icon_path'] = array(
					'field_class'			=> "IPS\\Helpers\\Form\\Text",
					'field_default'			=> NULL,
					'field_required'		=> FALSE,
					'field_extra'			=> array(),
					'field_hint'			=> "If provided, the conversion process will look in this location for any defined group images.",
	...
	public function finish()
	{
		...
		return array( "Search Index Rebuilding", "Caches Cleared", "Announcements Rebuilding", "Private Messages Rebuilding", "Signatures Rebuilding" );
	}
	...
	public function convert_emoticons()
	{
		...
		foreach( $this->fetch( 'smilies', 'sid' ) AS $row )
		{
			...
			$path		= rtrim( $this->app->_session['more_info']['convert_emoticons']['emoticon_path'], '/' ) . '/' . implode( '/', $filepath );
			...
			$libraryClass->convert_emoticon( $info, $set, $this->app->_session['more_info']['convert_emoticons']['keep_existing_emoticons'], $path );
			
			$libraryClass->setLastKeyValue( $row['sid'] );
		}
	}
	
	public function convert_warn_actions()
	{
		$libraryClass = $this->getLibrary();
		...
		foreach( $this->fetch( 'warninglevels', 'lid' ) AS $row )
		{
			...
			$libraryClass->convert_warn_action( array(
				'wa_id'				=> $row['lid'],
	...
	public function convert_warn_reasons()
	{
		$libraryClass = $this->getLibrary();
	...
	public function convert_profile_fields()
	{
		...
		foreach( $this->fetch( 'profilefields', 'fid' ) AS $row )
		{
			...
			$merge = $this->app->_session['more_info']['convert_profile_fields']["map_pfield_{$row['fid']}"] != 'none' ? $this->app->_session['more_info']['convert_profile_fields']["map_pfield_{$row['fid']}"] : NULL;
	...
	public function convert_groups()
	{
		...
		foreach( $this->fetch( 'usergroups', 'gid' ) AS $row )
		{
			...
			$icon_path = $this->app->_session['more_info']['convert_groups']['icon_path'];
			...
			$merge = $this->app->_session['more_info']['convert_groups']["map_group_{$row['gid']}"] != 'none' ? $this->app->_session['more_info']['convert_groups']["map_group_{$row['gid']}"] : NULL;
			
			$libraryClass->convert_group( $info, $merge, ( $icon_path ) ? rtrim( $icon_path, '/' ) . '/' . $row['image'] : NULL );
		...
		foreach( $this->fetch( 'users', 'uid' ) AS $row )
		{
			...
			switch( $row['avatartype'] )
			{
				...
				case 'upload':
					...
					$filename	= array_pop( $filepath );
					$filepath	= rtrim( $this->app->_session['more_info']['convert_members']['photo_location'], '/' ) . implode( '/', $filepath );
					break;
			}
	...
	public function convert_question_and_answers()
	{
		$libraryClass = $this->getLibrary();
		...
		foreach( $this->fetch( 'questions', 'qid' ) AS $row )
		{
			...
			$libraryClass->convert_question_and_answer( array(
				'qa_id'			=> $row['qid'],
