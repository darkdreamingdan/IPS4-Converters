...
class _Vbulletin5 extends \IPS\convert\Software
{
	...
	{
		return <<<INFORMATION
You can typically obtain these details from the file located at /path/to/vb5/core/includes/config.php
INFORMATION;
	}
			...
			case 'convert_emoticons':
				$return['convert_emoticons'] = array(
					'emoticon_path'				=> array(
						...
						'field_required'	=> TRUE,
						'field_extra'		=> array(),
						'field_hint'		=> "This is typically: /path/to/vb5/core/images/smilies",
	...
	public function convert_private_messages()
	{
		...
		foreach( $this->fetch( 'privatemessage', 'privatemessage.nodeid', array( "privatemessage.msgtype=? AND node.parentid=?", 'message', $node ) )->join( 'node', 'privatemessage.nodeid = node.nodeid' ) AS $pm )
		{
			try
			{
			...
			if ( $pm['oldid'] AND $this->db->checkForTable( 'pmtext' ) )
			{
				try
				{
					$pm['description'] = $this->db->select( 'message', 'pmtext', array( "pmtextid=?", $pm['oldid'] ) )->first();
				}
				catch( \UnderflowException $e ) {}
			}
			
			$posts = array();
			$posts[$pm['nodeid']] = array(
				'msg_id'			=> $pm['nodeid'],
				'msg_topic_id'		=> $pm['nodeid'],
				'msg_date'			=> $pm['publishdate'],
				'msg_post'			=> $pm['description'],
			...
			{
				if ( $post['oldid'] AND $this->db->checkForTable( 'pmtext' ) )
				{
					try
					{
						$post['description'] = $this->db->select( 'message', 'pmtext', array( "pmtextid=?", $pm['oldid'] ) )->first();
					}
					catch( \UnderflowException $e ) {}
				}
				
				$posts[$post['nodeid']] = array(
					'msg_id'			=> $post['nodeid'],
					'msg_topic_id'		=> $pm['nodeid'],
					'msg_date'			=> $post['publishdate'],
					'msg_post'			=> $post['description'],
			...
			foreach( $posts AS $post )
			{
				$maps[$post['msg_author_id']] = array(
					...
					'map_user_banned'		=> 0,
					'map_has_unread'		=> 0,
					'map_is_starter'		=> ( $post['msg_author_id'] == $pm['userid'] ) ? 1 : 0,
				);
			}
	...
	private function fetchType( $type )
	{
		...
		else
		{
			...
			{
				$this->typeCache[$contenttype['class']] = $contenttype['contenttypeid'];
			}
